...
const { mungeWeather, formatLocation, mungeYelpReviews } = require ('./mungeFile.js');
...
app.get('/reviews', async(req, res) => {
  try {
    ...
    const response = await response.get(URL).set('Authorization', `Bearer ${process.env.YELP_KEY}`);
    
    const newResponse = mungeYelpReviews(response.body);
